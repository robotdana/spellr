  ...
  def fetch(uri_str, uri_proc: URI.method(:parse), limit: 10) # rubocop:disable Metrics/MethodLength
    ...
    if limit == 0
      warn Spellr::StringFormat.red(
        "#{uri_str} was the last in an HTTP redirect chain that was too long"
      )
      ''
    end
    ...
    when Net::HTTPRedirection
      ...
      warn Spellr::StringFormat.red("#{uri_str} #{response.code}") if limit == 0
      ''
    end
  rescue StandardError => e
    warn Spellr::StringFormat.red("#{uri_str} #{e}")
    ''
  end
end
